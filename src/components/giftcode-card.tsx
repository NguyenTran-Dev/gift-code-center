  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-6 transition-all hover:bg-white/10">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">{giftcode.description || "Quà tặng tân thủ"}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 text-xs">
            {giftcode.expiryDate && (
              <div className="flex items-center gap-1.5 text-orange-400">
                ...
                <span>Hết hạn: {format(new Date(giftcode.expiryDate), "dd/MM/yyyy")}</span>
              </div>
            )}
        ...
        <Button
          onClick={handleCopy}
          className="w-full sm:w-auto bg-cyan-600 hover:bg-cyan-500 text-white gap-2 transition-all font-bold px-6 py-6 sm:py-2 rounded-xl"
        >
          {copied ? (
            <>
