...
import { loginAdmin, logoutAdmin, checkAuth } from "@/actions/auth-actions";
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0a0a0a] p-4">
        <div className="w-full max-w-md p-8 rounded-3xl border border-white/10 bg-[#121212] backdrop-blur-xl shadow-2xl animate-in fade-in zoom-in duration-500">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <Button
              type="submit"
              className="w-full h-12 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-xl shadow-lg shadow-cyan-500/20"
            >
              ĐĂNG NHẬP HỆ THỐNG
            </Button>
          </form>
    ...
    <div className="flex min-h-screen bg-[#0a0a0a] text-white">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 border-b border-white/10 flex items-center justify-between px-8 bg-[#0c0c0c]">
          <h2 className="text-sm font-bold uppercase tracking-widest text-gray-500 italic">
            Bảng điều khiển <span className="text-cyan-400">Hệ thống</span>
          </h2>
